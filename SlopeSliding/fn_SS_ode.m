...
% dndx = H.*k./2.*cos(-omega.*t + f.c); % SURGE ONLY (NO DRIFT)
if trans == 1 % INCLUDE TRANSIENTS
 dndx = 1e-3*f.a.*t.^f.b.*k*cos(k.*X(1) - omega.*t + f.c);
%  dndx = 1e-3*f.a.*t.^f.b.*k*cos(-omega.*t + f.c); % SURGE ONLY (NO DRIFT)
end
...
% % ADD STOKES DRIFT (DEEP WATER APPROXIMATION)
% Vw = Vw + omega.*k.*(H./2).^2;
